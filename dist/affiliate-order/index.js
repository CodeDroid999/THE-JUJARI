(() => {
  ...
  new Vue({
    ...
    data: { userId: "", loading: !0, orders: { data: [] } },
    methods: {
      getOrders() {
        const a = new URLSearchParams(location.search).get("uid");
        (this.userId = a),
          t
            .get(
              `${e.affiliateOrderList}${a ? "?subordinate_user_id=" + a : ""}`
            )
            .then((e) => {
      ...
      handlePageClick(e, a) {
        let i = e;
        e.includes("?")
          ? (i += this.userId ? `&subordinate_user_id=${this.userId}` : "")
          : (i += this.userId ? `?subordinate_user_id=${this.userId}` : ""),
          (this.loading = !0),
          t.get(i).then((e) => {
            if (0 === e.code)
              if ("replace" === a) this.orders = e.data;
              else if ("push" === a) {
                const a = e.data,
                  t = [...this.orders.data, ...e.data.data];
                (a.data = t), (this.orders = a);
              }
            this.loading = !1;
          });
      },
      getEmptyContainerHeight() {
        ...
        if (window.innerWidth < 1280) {
          const a = document.getElementById("header").clientHeight,
            t = document.getElementById("footer").clientHeight;
          e = window.innerHeight - a - t;
        } else {
          e = document.querySelector(".user-sidenav").clientHeight;
        }
        return e;
      },
    },
