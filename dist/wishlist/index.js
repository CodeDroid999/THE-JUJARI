(() => {
      ...
      computed: {
        formatPrice() {
          ...
          return this.isProduct
            ? `${e.symbol}${r.formatNumber(this.price)}`
            : `${e.symbol}${r.formatNumberWithoutUnit(this.price)}`;
        },
      },
      template:
  ...
  new Vue({
    ...
    data: { list: { data: [] }, loading: !0, viewAlert: !1, alertMessage: "" },
    methods: {
      getProducts() {
        (this.loading = !0),
          i.get(e.wish).then((e) => {
            0 === e.code && (this.list = e.data), (this.loading = !1);
          });
      },
      deleteWish(t) {
        i.delete(`${e.wish}/${t}`).then((e) => {
          0 === e.code
            ? this.getProducts()
